                ...
                <div className="flex gap-4">
                    <div className="p-6 bg-white/5 border border-white/10 rounded-3xl">
                        ...
                        <p className="text-2xl font-black">
                            KSh {Math.floor(allPlans.reduce((acc: number, curr: any) => acc + curr.amountPaid, 0)).toLocaleString()}
                        </p>
                    </div>
                </div>
            </header>
                    ...
                    {completedPlans.length === 0 ? (
                        ...
                        completedPlans.map((plan: any) => (
                            <CompletedRow
                                ...
                                amount={`KSh ${Math.floor(plan.amountPaid).toLocaleString()}`}
                            />
                        ))
                    )}
                </div>
...
function BNPLCard({ id, name, total, paid, image }: { id: string, name: string; total: number; paid: number; image: string }) {
    ...
    const initiateMpesa = useAction(api.bnpl_payments.initiateBnplMpesaPayment);
    ...
    const handlePayment = async () => {
        ...
        if (paymentAmount > remaining) {
            alert(`Amount cannot exceed remaining balance of KSh ${Math.floor(remaining)}`);
            return;
        }
        ...
        try {
            ...
            if (result.ResponseCode === "0") {
                ...
                setLoading(false);
                // Poll for payment confirmation (in production, this would be handled by the callback)
                alert("M-Pesa prompt sent! Please check your phone and enter your PIN to complete payment.");
            } else {
    ...
    const handleCancel = async () => {
        if (confirm(`Are you sure you want to cancel this savings plan?\n\nYou have saved KSh ${Math.floor(paid)}. This amount will be refunded to your wallet.`)) {
            try {
                await cancelPlan({ planId: id as any });
                setOpen(false);
                alert("Plan cancelled successfully. Your savings have been refunded.");
            } catch (e) {
                console.error(e);
            ...
            <div className="flex-1 space-y-6">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-sm text-white font-bold">Total Target: KSh {Math.floor(total).toLocaleString()}</p>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-white/5">
                    <div>
                        ...
                        <p className="text-xl font-black">KSh {Math.floor(paid).toLocaleString()}</p>
                    </div>
                    <div className="text-right">
                        <p className="text-[10px] font-black text-white uppercase">Remaining</p>
                        <p className="text-xl font-black text-white">KSh {Math.floor(remaining).toLocaleString()}</p>
                    </div>
                </div>

                <button
                    onClick={() => setOpen(true)}
                    className="w-full py-4 bg-green-600 text-white font-black rounded-2xl flex items-center justify-center gap-2 hover:bg-green-500 active:scale-[0.98] transition-all shadow-lg shadow-green-600/20"
                >
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
                        <div className="bg-[#111] border border-white/10 rounded-[32px] w-full max-w-md p-8 animate-in fade-in zoom-in-95 duration-200 relative">
                            ...
                            <div className="mb-6">
                                ...
                                <p className="text-white text-sm mt-1">Add funds via M-Pesa to your savings jar.</p>
                            </div>
                            ...
                            {awaitingConfirmation ? (
                                <div className="space-y-6 text-center py-8">
                                    ...
                                    <div>
                                        <p className="text-lg font-black mb-2">Awaiting Payment Confirmation</p>
                                        <p className="text-sm text-white">Please complete the payment on your phone.</p>
                                        <p className="text-xs text-white mt-4">This may take a few moments...</p>
                                    </div>
                                    <button
                                        onClick={() => {
                                            setOpen(false);
                                        ...
                                        className="text-white text-xs font-bold uppercase hover:text-white/70"
                                    >
                                        Close
                                    </button>
                                </div>
                                ...
                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold uppercase tracking-widest text-white">Amount (KSh) - Max: KSh {Math.floor(remaining).toLocaleString()}</label>
                                        <input
                                            type="number"
                                            value={amount}
                                            onChange={(e) => {
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold uppercase tracking-widest text-white">M-Pesa Phone Number</label>
                                        <input
                                            type="tel"
                                            value={phoneNumber}
                                        ...
                                        />
                                        <p className="text-[10px] text-white uppercase font-bold">Enter the number to receive the M-Pesa prompt</p>
                                    </div>

                                    <button
                                        onClick={handlePayment}
                                        disabled={loading || !amount || !phoneNumber}
                                        className="w-full py-4 bg-green-600 text-white font-black rounded-xl hover:bg-green-500 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
                                    >
                                    ...
                                    >
                                        Cancel Plan & Refund KSh {Math.floor(paid)}
                                    </button>

                                    <p className="text-[10px] text-white text-center font-bold uppercase">
                                        Secured by M-Pesa Daraja API • Real Payments
                                    </p>
                                </div>
                            )}
                        </div>
